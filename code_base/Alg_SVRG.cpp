...
mxArray* SVRG_dense(int nlhs, const mxArray *prhs[]) {
	...
	for (k = 0; k < iters_outer; k++)
	{
		...
		compute_full_gradient(Xt, w, y, mu, n, d, lambda);
		// Initiation
		for (j = 0; j < d; j++) {
			w_prev[j] = w[j];
			mu[j] += lambda * w[j];
		...
		for (i = 0; i < iters_inner; i++) {
			...
			info = compute_info_dense(Xt, w, y, d, idx);
			info_prev = compute_info_dense(Xt, w_prev, y, d, idx);

			for (j = 0; j < d; j++) {
				w[j] -= alpha * (mu[j] + Xt[idx*d + j] * (info - info_prev) + 
						lambda * (w[j] - w_prev[j]));
			}
		}
	}
	...
	for (k = 0; k < iters_outer; k++)
	{
		...
		for (i = 0; i < iters_inner; i++) {
			...
			for (long j = jc[idx]; j < jc[idx+1]; j++) {
				w[ir[j]] -= alpha * (i - last_seen[ir[j]]) *
					(mu[ir[j]] + lambda * (w[ir[j]] - w_prev[ir[j]]));
				last_seen[ir[j]] = i;
			}
		...
		for (long j = 0; j < d; j++) {
			w[j] -= alpha * (iters_inner - last_seen[j]) *
				(mu[j] + lambda * (w[j] - w_prev[j]));
		}
	}
